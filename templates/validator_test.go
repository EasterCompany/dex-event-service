...
func TestGetTemplateList(t *testing.T) {
	...
	if len(eventTypes) != 45 {

		t.Errorf("Expected 45 event types, got %d", len(eventTypes))
	...
	expectedTypes := map[string]bool{
		...
		"system.cognitive.model_inference": true,
		...
		string(types.EventTypeCLIInteraction): true,
		...
		string(types.EventTypeSystemTestCompleted): true,

		string(types.EventTypeSystemBuildCompleted): true,

		string(types.EventTypeSystemStatusChange): true,

		string(types.EventTypeSystemDiagnosticPing): true,

		"system.fabricator.voice_trigger":  true,
		"system.fabricator.voice_complete": true,
